...
class Exercise2:
    ...
    def run(cls,path):
        ...
        df_20170101['mean']=df_20170101.groupby('type')['dongfour'].apply(lambda x : pd.Series(data=x.sum(),index=x.index))
        hourList=list(set(df_20170101['hour']))
        df_1=pd.DataFrame(data=hourList)
        df_1['avg']=df_20170101.groupby('type')['dongfour'].apply(lambda x : pd.Series(data=x.sum(),index=x.index))
        print df_1.index
        print df_1
